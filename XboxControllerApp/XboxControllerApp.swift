...
import Combine
import UserNotifications // Necessário para notificações
...
struct XboxControllerApp: App {
    ...
    var body: some Scene {
        ...
        // 2. Item na Barra de Menus (MenuBar)
        // Isso cria o ícone perto do relógio do Mac
        MenuBarExtra("Xbox Driver", systemImage: "gamecontroller.fill") {
            Text("Xbox Wireless Driver").font(.caption)
            Divider()
            
            if driverManager.connectedControllers.isEmpty {
                Text("Nenhum controle conectado")
            } else {
                ForEach(driverManager.connectedControllers) { ctrl in
                    HStack {
                        // Ícone muda se for Xbox 360 ou One/Series
                        Image(systemName: "gamecontroller")
                        Text(ctrl.name)
                        // Mostra a bateria no menu também
            ...
            Button("Abrir Dashboard") {
                ...
                // Se a janela foi fechada, reabre
...
final class DriverManager: NSObject, ObservableObject {
    ...
    private let extensionIdentifier = "com.xboxcontroller.driver"
    
    override init() {
        super.init()
        
        // Solicita permissão para enviar notificações (Pop-ups no canto da tela)
        ...
        // (OPCIONAL) Controle de teste para ver a UI sem hardware - remova em produção
        // let debugCtrl = ControllerInfo(name: "Debug Xbox 360", type: .xbox360, isConnected: true, batteryLevel: 75, vendorID: "0", productID: "0", serialNumber: nil)
    ...
    private func sendNotification(title: String, body: String) {
        ...
        // Cria um ID único para não substituir notificações anteriores
    ...
    private func registerController(_ controller: GCController) {
        ...
        DispatchQueue.main.async {
            ...
            self.sendNotification(
                title: String(localized: "Controle Conectado"),
                body: "\(newInfo.name) está pronto para jogar."
            )
        }
    ...
    private func unregisterController(_ controller: GCController) {
        if let (uuid, _) = gcControllersMap.first(where: { $0.value == controller }) {
            ...
            DispatchQueue.main.async {
                let name = self.connectedControllers.first(where: {$0.id == uuid})?.name ?? "Controle"
                self.connectedControllers.removeAll { $0.id == uuid }
                self.statusMessage = String(localized: "Controle desconectado")
                
                // Notifica o usuário
                self.sendNotification(
                    title: String(localized: "Controle Desconectado"),
                    body: "A conexão com \(name) foi perdida."
                )
            }
        }
    }
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            if let selectedController = driverManager.connectedControllers.first {
                ScrollView {
                    // Visualizador do controle (Joystick)
            ...
            if controllers.isEmpty {
                ...
                ForEach(controllers) { ctrl in
                    HStack {
                        ...
                        Spacer()
                        // AQUI ESTÁ A MUDANÇA: Usamos o novo BatteryView
                        if let batt = ctrl.batteryLevel {
                            BatteryView(level: batt)
                        }
                    }
                    .padding(5)
                    .background(Color.gray.opacity(0.1))
...
struct FooterView: View {
    var body: some View {
        Text("v1.1.0 - DriverKit & Menu Bar Support")
            .font(.caption2)
            .foregroundColor(.secondary)
            .padding(.top, 10)
    }
